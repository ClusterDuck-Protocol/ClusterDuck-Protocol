...
bool DuckPacket::update(std::vector<byte> duid, std::vector<byte> dataBuffer) {
  ...
  logdbg("Updating received packet: " +
         String(duckutils::convertToHex(dataBuffer.data(), packet_length)));
  ...
  if (path_pos > (packet_length - 1)) {
    logerr("ERROR Path position is invalid (" + String(path_pos) +
           ") Data may be corrupted.");
    return false;
  }
  ...
  packet.reserved.insert(packet.reserved.end(), &dataBuffer[RESERVED_POS],
                         &dataBuffer[DATA_POS]);
  packet.data.insert(packet.data.end(), &dataBuffer[DATA_POS],
                     &dataBuffer[path_pos]);
  ...
  logdbg("Current path: " + String(duckutils::convertToHex(
                                packet.path.data(), packet.path.size())));

  // check if we need to relay the packet
  relaying = relay(duid);
  logdbg("Updated path: " + String(duckutils::convertToHex(
                                packet.path.data(), packet.path.size())));
  logdbg("Updated buffer: " +
         String(duckutils::convertToHex(buffer.data(), buffer.size())));
  return relaying;
}
...
int DuckPacket::buildPacketBuffer(byte topic, std::vector<byte> app_data) {
  ...
  // logdbg("Duid:      " + String(duckutils::convertToHex(duid.data(),
  // duid.size())));

  // message uid
  buffer.insert(buffer.end(), &message_id[0], &message_id[MUID_LENGTH]);
  // logdbg("Muid:      " + String(duckutils::convertToHex(buffer.data(),
  // buffer.size())));

  // topic
  buffer.insert(buffer.end(), topic);
  // logdbg("Topic:     " + String(duckutils::convertToHex(buffer.data(),
  // buffer.size())));

  // path offset
  byte offset = HEADER_LENGTH + app_data_length;
  buffer.insert(buffer.end(), offset);
  // logdbg("Offset:    " + String(duckutils::convertToHex(buffer.data(),
  // buffer.size())));

  // reserved
  buffer.insert(buffer.end(), 0x00);
  buffer.insert(buffer.end(), 0x00);
  // logdbg("Reserved:  " + String(duckutils::convertToHex(buffer.data(),
  // buffer.size())));

  // ----- insert data -----
  buffer.insert(buffer.end(), app_data.begin(), app_data.end());
  // logdbg("Data:      " + String(duckutils::convertToHex(buffer.data(),
  // buffer.size())));

  // ----- insert path -----
  buffer.insert(buffer.end(), duid.begin(), duid.end());
  // logdbg("Path:      " + String(duckutils::convertToHex(buffer.data(),
  // buffer.size())));
