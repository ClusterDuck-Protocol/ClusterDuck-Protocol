...
3. Run the examples for the Lilygo Tbeam (with SX1276 LoRa chip) using your local CDP library, supplying the relative path to the example folder you want to use. If no EXAMPLE_DIR is supplied, it defaults to Basic-Ducks/MamaDuck
    ```bash
    EXAMPLE_DIR=Basic-Ducks/DuckLink platformio run -e local_lilygo_t_beam_sx1276 -t upload
    ```

4. Run the examples for the Lilygo Tbeam (with SX1276 LoRa chip) using the CDP library from the PlatformIO library registry.
    ```bash
