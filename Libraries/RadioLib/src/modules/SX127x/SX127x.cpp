...
int16_t SX127x::readData(uint8_t* data, size_t len) {
  ...
  if(modem == SX127X_LORA) {
    ...
    if(regValue == SX127X_CLEAR_IRQ_FLAG_PAYLOAD_CRC_ERROR || crcOnPayload == 0) {
      // clear interrupt flags
      RADIOLIB_DEBUG_PRINTLN("===> [RL] readData. WARNING CRC error. CrcOnPayload: " + String(crcOnPayload));
